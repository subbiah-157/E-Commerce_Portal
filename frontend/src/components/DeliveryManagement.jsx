...
import { 
  ...
  FaClipboardList, FaShoppingCart, FaMapMarkerAlt, FaPhone, FaEnvelope, FaUser
} from "react-icons/fa";
...
const DeliveryManagement = () => {
  ...
  // ✅ Fetch ONLY orders that were sent to delivery (sentToDelivery: true)
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get("http://localhost:5000/api/orders/delivery");
        const deliveryOrders = response.data.filter((o) => o.sentToDelivery === true);
  ...
  const handleMarkDelivered = async (orderId) => {
    try {
      ...
      const invoiceRes = await axios.post(
        `http://localhost:5000/api/invoices/generate/${orderId}`
      );
      ...
      if (order) {
        const templateParams = {
          user_name: order.userDetails?.name || "Customer",
          user_email: order.userDetails?.email,
          order_id: order._id,
          ...
          products: order.products
            .map(
              (p) =>
                `${p.name} (Qty: ${p.qty}, Total: ${p.total} ${order.currency})`
            )
            .join("\n"),
          delivery_date: new Date(updatedOrder.deliveredAt).toLocaleString("en-IN", {
  ...
  const renderProductImage = (product) => {
    if (product.image) {
      return <img src={product.image} alt={product.name} className="smc-product-image" />;
    }
    return <div className="smc-product-image-placeholder"><FaImage /></div>;
  };
...
// ✅ OrderCard
const OrderCard = ({ order, expandedOrders, expandedProducts, toggleOrderDetails, toggleProductDetails, handleMarkDelivered, renderSize, renderProductImage, isCompleted }) => {
        ...
        <div className="smc-order-card-details">
                ...
                return (
                  <div key={idx} className="smc-product-item">
                    <div className="smc-product-summary" onClick={() => toggleProductDetails(order._id, idx)}>
                      <div className="smc-product-image-container">{renderProductImage(p)}</div>
          ...
          {isCompleted && order.deliveredAt && (
            <div className="smc-delivery-info">
              <h4><FaShippingFast className="smc-section-icon" />Delivery Information</h4>
              <div className="smc-detail-item">
                <span className="smc-detail-label">Delivered On:</span>
