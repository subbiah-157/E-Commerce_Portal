...
const CategoryManagement = ({ categories, fetchCategories }) => {
  ...
  const handleAddSubCategory = async () => {
    ...
    try {
      await axios.post("http://localhost:5000/api/categories/sub", {
  ...
  const handleUpdateSub = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/categories/${editingSubCatParentId}/sub/${editingSubId}`,
        { name: editingSubName }
      );
  ...
  const handleDeleteSub = async (categoryId, subCategoryId) => {
    try {
      await axios.delete(`http://localhost:5000/api/categories/${categoryId}/sub/${subCategoryId}`);
      fetchCategories();
    } catch (err) {
  ...
  const handleAddSubSub = async () => {
    ...
    try {
      await axios.post(
        `http://localhost:5000/api/categories/${parentCategory._id}/sub/${selectedSubId}/subsub`,
        { subSubCategoryName }
      );
  ...
  const handleUpdateSubSub = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:5000/api/categories/${editingSubSubParentId.categoryId}/sub/${editingSubSubParentId.subCategoryId}/subsub/${editingSubSubIndex}`,
  ...
  const handleDeleteSubSub = async (parentId, subId, index) => {
    try {
      await axios.delete(`http://localhost:5000/api/categories/${parentId}/sub/${subId}/subsub/${index}`);
      fetchCategories();
    } catch (err) {
